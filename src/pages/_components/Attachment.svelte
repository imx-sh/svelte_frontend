<script>
  ...
  export let data;
	...
	let author = '';
	if("author" in data.attributes)
		author = data.attributes.author.shortname;
	
	let datetime = '';
	if(( "created_at" in data.attributes ) && data.attributes.created_at) {
		//console.log( "x[", data.shortname, "]x >> y[", data.attributes.created_at, "]y");
		datetime = $date(Date.parse(data.attributes.created_at.replace(" ", "T").replace(" UTC", "").replace(" ", "")), {format: "short"});
	}
...
{/if}
<Icon name="user" /> {author} <Icon name="clock" /> {datetime} <br/>
