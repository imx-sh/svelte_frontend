...
<Card class="mb-4" >
  <CardHeader class="py-0 ">
		<CardTitle class="float-start ps-2 mt-1 mb-1">
			...
			{displayname}
			{:else}
			<CardLink href={$url("/entry/:subpath/:shortname", {subpath: data.subpath, shortname: data.shortname})} title={$_('more')} > {displayname} </CardLink>
			{/if}

		</CardTitle>
		<CardTitle class="float-end mb-0 pt-1">
    <Button href="#" title="Media" size="sm" outline >
			<Icon name="paperclip" /> <Badge color="secondary" >{$number(data.attachments.media ? data.attachments.media.length : 0)}</Badge>
    </Button>
    <Button href="#" title="Like" on:click={handle("like")} size="sm" color="success" outline >
			<Icon name="heart" /> <Badge color="secondary" >{$number(data.attachments.reaction ? data.attachments.reaction.length : 0)}</Badge>
    </Button>
    <Button href="#" title="Comment" on:click={handle("comment")} size="sm" outline >
      <Icon name="reply-all" /> <Badge color="secondary">{$number(data.attachments.reply ? data.attachments.reply.length : 0)}</Badge>
    </Button>
    <Button href="#" title="Share" on:click={handle("share")} size="sm" outline >
      <Icon name="share" /> <Badge color="secondary">{$number(data.attachments.share ? data.attachments.share.length : 0)}</Badge>
    </Button>
    ...
    <Button href="#" title="Unpublish" on:click={handle("report")} size="sm" outline >
      <Icon name="eye" /> 
		...
		<!--Button href="#" title="Edit" on:click={handle("some")} size="sm" outline color={changed?"danger":"light"}>
      <Icon name="upload" /> 
    </Button>
    <Button href="#" title="Delete" on:click={handle("report")} size="sm" outline >
      <Icon name="bin" /> 
    </Button-->
		{/if}
		</CardTitle>
  ...
  <CardBody class="pb-1">
    <CardText class="border-bottom">
			<!--div bind:this={maindiv} on:click={showEditor}-->
				...
				{:else if entryurl}
					<MediaView url={entryurl} {displayname} {content_type} /> <!--description={data.attributes.description} /-->
				{/if}
			<!--/div-->
    </CardText>
    <CardSubtitle class="">
      ...
      {$_('tags')}: 
			...
			{/each}
      <Icon name="clock" />
       {$_('edited_at')}: <abbr title={$_('datetime', {values:{ ts: timestamp}})}> {$date(timestamp, {format: "short"})} </abbr>
			</small>
    </CardSubtitle>
  </CardBody>

	<!--Attachments subpath={data.subpath} parent_shortname={data.shortname} attachments={data.attachments} {extended} /-->
</Card>
