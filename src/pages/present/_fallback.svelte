<script>
  ...
  imx_pub_query( subpath, resourceTypes, [], "subpath", "*", 30, 0).then( (resp) => {
		items = resp.records;
...
<Container class="h-100 border border-info " fluid={true} >
  ...
  <Row class="border-success mt-2" > 
		<Col class="text-center"> 
			<span class="h3" > {folder.attributes.displayname} <!--b>i</b>nformation <b>m</b>anagement and e<b>x</b>change--> </span> : 
...
<div bind:clientHeight={footerHeight} class="fixed-bottom" >
  <Row class="border align-items-end align-self-end py-2">
		...
		<Col class=" text-end pe-4">
			<small> {$_("page_of_total", {values:{page: $number(currentIndex+1), total: $number(items.length)}})} </small>
		</Col>
